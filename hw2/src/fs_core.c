...
char get_block_k(FILE* fs, struct INode *inode, block_pointer_t k, block_pointer_t* block_p_holder) {
    ...
    while (level > 0) {
        ...
        fseek(fs, AREA_POS_BLOCKS + BLOCK_SIZE * block_p, SEEK_SET);
        fread(block_temp, BLOCK_SIZE, 1, fs);
...
char is_block_allocated(FILE *fs, block_pointer_t block_p) {
    ...
    long sz = ftell(fs);
    if ((AREA_POS_BLOCKS + BLOCK_SIZE * block_p) < sz) {
        return 1;
    } else {
        return 0;
    }
...
char inode_block_append(FILE *fs, struct INode *inode, block_pointer_t *block_p_holder) {
    ...
    while (level > 1) {
        ...
        if (k == 0) {
            if (!(occupy_block(fs, &new_block_p))) {
                fseek(fs, AREA_POS_BLOCKS + block_p * BLOCK_SIZE + p * sizeof(block_pointer_t), SEEK_SET);
    ...
    if (!(occupy_block(fs, &new_block_p))) {
        fseek(fs, AREA_POS_BLOCKS + block_p * BLOCK_SIZE + k * sizeof(block_pointer_t), SEEK_SET);
        fwrite(&new_block_p, sizeof(new_block_p), 1, fs);
        inode->file_size += 1;
        if (block_p_holder != NULL) {
            *block_p_holder = new_block_p;
...
void get_block(FILE* fs, block_pointer_t block_p, char* block_holder) {
    fseek(fs, AREA_POS_BLOCKS + block_p * BLOCK_SIZE, SEEK_SET);
    fread(block_holder, BLOCK_SIZE, 1, fs);
}
...
void print_full_path(FILE *fs, inode_pointer_t inode_p) {
    unsigned int level = 0;
    ...
    while (inode_p2 != 0) {
        if (get_parent_directory(fs, inode_p2, &inode_p2)) {
            fprintf(stderr, "Error while getting full path (%d) [#1]\n", inode_p);
            return;
        }
        ++level;
    }
...
size_t get_regular_file_size(FILE *fs, struct INode *inode) {
    ...
    size_t eof_pos;
    size_t sz = 0;
    if (inode->file_type != TYPE_REGULAR) return BLOCK_SIZE * get_size_on_disk(inode);
    if (inode->file_size == 0) return sz;
    sz += (inode->file_size - 1) * BLOCK_SIZE;
    if (get_block_k(fs, inode, inode->file_size - 1, &block_p)) return sz;
    get_block(fs, block_p, block);
    for (eof_pos = 0; (eof_pos < BLOCK_SIZE) && block[eof_pos] != EOF; ++eof_pos) {}
    sz += eof_pos;
    return sz;
}
